...
static struct gpiod_line_request *pwm_req, *fwd_req, *bwd_req;
...
int main(void) {
    ...
    fwd_req = request_output_line(chip, FORWARD_PIN,  0, "xdc-forward");
    bwd_req = request_output_line(chip, BACKWARD_PIN, 0, "xdc-backward");
    ...
    if (!pwm_req || !fwd_req || !bwd_req || !sw_minus_req || !sw_plus_req) {
