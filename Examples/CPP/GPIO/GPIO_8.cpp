...
constexpr unsigned int DIR_PIN = 27;
constexpr unsigned int ENABLE_PIN = 22;
...
void enable(gpiod::line_request& req) {
    set(req, ENABLE_PIN, true);
}

void disable(gpiod::line_request& req) {
    set(req, ENABLE_PIN, false);
}

void apply_direction(gpiod::line_request& req) {
    if (direction == 1) {
        set(req, DIR_PIN, true);
        std::cout << "Direction: +" << std::endl;
    } else {
        set(req, DIR_PIN, false);
        std::cout << "Direction: -" << std::endl;
    }
}
